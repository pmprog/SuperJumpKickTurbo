...
void TransitionTiled::PrepareTiles( TiledTransitions::Transition Style, int TilesWide, int TilesHigh )
{
	frameIndex = 0;
	tileWide = TilesWide;
	tileWidth = DISPLAY->GetWidth() / TilesWide;
	if( (tileWidth * TilesWide) < DISPLAY->GetWidth() )
	{
		tileWidth++;	// Fix rounding
	}
	tileHigh = TilesHigh;
	tileHeight = DISPLAY->GetHeight() / TilesHigh;
	if( (tileHeight * TilesHigh) < DISPLAY->GetHeight() )
	{
